...
import { MessageCircle, BarChart2, Wind, BookOpen, Moon, Sun, LogOut, X, Menu } from 'lucide-react';

const Sidebar = ({ activePage, setActivePage, isDarkMode, setIsDarkMode, onSignOut, isOpen, setIsOpen }) => {
    const navItems = [
        ...
        <>
            ...
            <div
                className={`
                    ...
                    lg:translate-x-0
                    h-screen border-r border-gray-50 dark:border-gray-700/50
                    flex flex-col pt-10 pb-8 px-6
                    shadow-[10px_0_40px_rgba(0,0,0,0.02)] relative
                `}
            >
                {/* Close button — mobile only */}
                <button
                    onClick={() => setIsOpen(false)}
                    className="absolute top-4 right-4 lg:hidden p-2 rounded-xl text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                >
                    <X size={20} />
                </button>
